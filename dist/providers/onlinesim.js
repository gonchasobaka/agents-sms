...
exports.OnlineSimProvider = void 0;
...
const ISO_TO_PHONE = {
    ...
    cy: 357, cz: 420, dk: 45, dj: 253, do: 1, ec: 593, eg: 20, sv: 503, gq: 240,
    ...
    jm: 1, jp: 81, jo: 962, kz: 7, ke: 254, kp: 850, kr: 82, kw: 965, kg: 996,
...
// Country name (english, lowercase) → phone code for convenience
const NAME_TO_PHONE = {
...
/**
 * Convert any country input to OnlineSim phone code.
 * Accepts: numeric string ("372"), ISO-2 ("ee"), country name ("estonia"), "any".
 * Returns undefined for "any"/unknown.
 */
function toPhoneCode(country) {
    if (!country || country === "any")
        return undefined;
    const lower = country.toLowerCase().trim();
    // Already numeric
    const num = parseInt(country, 10);
    if (!isNaN(num) && String(num) === country.trim())
        return num;
    // ISO-2 lookup
    if (ISO_TO_PHONE[lower])
        return ISO_TO_PHONE[lower];
    // Name lookup
    if (NAME_TO_PHONE[lower])
        return NAME_TO_PHONE[lower];
...
class OnlineSimProvider {
    ...
    async buyNumber(service, country) {
        ...
        if (data.response === "UNDEFINED_COUNTRY") {
            throw new Error(`onlinesim: invalid country '${country}'. Use phone codes (1=USA, 49=Germany, 33=France)`);
        }
    ...
    async getServices(country) {
        // Resolve to phone codes; for "any" query a popular set
        let phoneCodes;
        if (!country || country === "any") {
...
exports.OnlineSimProvider = OnlineSimProvider;
